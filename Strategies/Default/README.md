...
An AI-assisted stock screener for the Indian market (NSE/BSE). You describe a
...
against a dynamically built stock universe to produce a ranked shortlist.
...
- **`rules.md`** — the strategy spec in human-readable form: mandatory filters,
  ...
  this file to change or create a strategy.
- **`generate_screener.py`** — reads `rules.md`, sends it to the Gemini API,
  ...
  function to `generated_rules.py`.
...
- **`universe_fetcher.py`** — downloads the Nifty 500 constituent list from
  NSE, pulls 6-month price history via `yfinance`, ranks stocks by sector
...
- **`stock_screener.py`** — the main entry point. Builds the stock universe,
  fetches 1-year daily price data for each stock, runs `generated_rules.py`'s
  `evaluate_stock` on each one, and writes the ranked results to
  `stock_report.csv`.
...
1. **Install dependencies:**
   ...
   (`python-dotenv` and `google-genai` are needed by `generate_screener.py`
   but aren't currently listed in `requirements.txt`.)
...
### 1. Define or edit your strategy

Edit `rules.md` to describe your filters, ranking formula, and desired output
columns. The current strategy ("Trendline Support Momentum") looks for stocks:
...
### 2. Generate the screening logic

```bash
python generate_screener.py
```

This calls Gemini with `rules.md` as the spec and writes the generated
`evaluate_stock` function to `generated_rules.py`.
...
- Download 1-year daily price history for each stock via `yfinance`.
...
- `universe_cache.json` expires after 1 day; delete it to force a fresh
  fetch of the Nifty 500 universe.
- `calculate_sector_strength` in `stock_screener.py` currently uses a
  **hardcoded** sector ranking for demonstration purposes rather than a real
  calculation — update it if you need genuine sector-strength ranking.
